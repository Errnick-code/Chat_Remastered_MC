...
public final class ImageUploadFlow {
    ...
    public static void stageImage(File file) {
        ...
        if (PendingImageState.size() >= maxPhotos) {
            mc.gui.getChat().addMessage(Component.literal("§e[Chat Remastered] " + ChatRemasteredConfig.tr("chat-remastered.max_photos_reached", maxPhotos)));
            return;
        }
        ...
        Thread thread = new Thread(() -> {
            try {
                ...
                mc.execute(() -> {
                    ...
                    mc.gui.getChat().addMessage(Component.literal("§c[Chat Remastered] " + ChatRemasteredConfig.tr("chat-remastered.error", (Object) (e.getMessage() != null ? e.getMessage() : "?"))));
                });
            }
        });
        ...
        mc.execute(() -> {
            ...
            try {
                if (finalPreviewBytes.length > 0) {
                    ...
                    ResourceLocation previewId = ResourceLocation.fromNamespaceAndPath("chat-remastered", "preview_" + System.currentTimeMillis());
                    ...
                    PendingImageState.updateTextureForUid(uid,
                            ResourceLocation.fromNamespaceAndPath("chat-remastered", "gif_pending"),
    ...
    public static void sendPendingImageWithCaptionAndReply(String caption, String replyToSender, String replyToText, String replyToImageId) {
        ...
        String sender = player.getGameProfile().getName();
        ...
        Thread thread = new Thread(() -> {
            ...
            switch (result) {
                ...
                default -> mc.execute(() -> {
                    ...
                    mc.gui.getChat().addMessage(Component.literal("§c[Chat Remastered] " + ChatRemasteredConfig.tr("chat-remastered.upload_error", (Object) result)));
                });
            }
        });
        thread.setDaemon(true);
    ...
    public static void pasteImageFromClipboard() {
        ...
        Thread thread = new Thread(() -> {
            try {
                ...
                mc.execute(() -> {
                    PendingImageState.removeByUid(uid);
                    mc.gui.getChat().addMessage(Component.literal("§c[Chat Remastered] " + ChatRemasteredConfig.tr("chat-remastered.clipboard_error", (Object) (e.getMessage() != null ? e.getMessage() : "?"))));
                });
            }
        });
        thread.setDaemon(true);
