...
public final class ScreenshotsPanel {
    ...
    public void render(GuiGraphicsExtractor graphics, int screenW, int screenH, int mouseX, int mouseY) {
        tick();
        if (!isOpen()) return;
        ...
        graphics.text(mc.font, title, px + PADDING, titleY, 0xFFFFFFFF, false);
        ...
        graphics.centeredText(mc.font, "✕", closeX + CLOSE_SIZE / 2, closeY + (CLOSE_SIZE - mc.font.lineHeight) / 2, closeColor);
        ...
        if (!scanned && scanning) {
            graphics.text(mc.font, ChatRemasteredConfig.tr("chat-remastered.screenshots_loading"),
                    listLeft, listTop, 0xFFAAAAAA, false);
        } else if (scanned && entries.isEmpty()) {
            graphics.text(mc.font, ChatRemasteredConfig.tr("chat-remastered.screenshots_empty"),
                    listLeft, listTop, 0xFFAAAAAA, false);
        } else {
    ...
    private void cr$renderFolderBar(GuiGraphicsExtractor graphics, Minecraft mc, int px, int pw, int screenW, int screenH, int mouseX, int mouseY) {
    ...
    private void renderTile(GuiGraphicsExtractor graphics, Minecraft mc, Entry e, int x, int y, int w, int mouseX, int mouseY) {
        ...
        if (e.textureId != null) {
            ...
            graphics.centeredText(mc.font, "?", x + w / 2, y + th / 2 - mc.font.lineHeight / 2, 0xFF888888);
        } else {

            graphics.centeredText(mc.font, "🖼", x + w / 2, y + th / 2 - mc.font.lineHeight / 2, 0xFF666666);
        }
        ...
        graphics.text(mc.font, caption, x, y + th + 2, 0xFF999999, false);
    }
    ...
    private void cr$renderContextMenu(GuiGraphicsExtractor graphics, int screenW, int screenH, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < labels.length; i++) {
            ...
            graphics.text(mc.font, icons[i], mx0 + CTX_PAD + 2, textY, textColor, false);
            graphics.text(mc.font, labels[i], mx0 + CTX_PAD + CTX_ICON_COL, textY, textColor, false);
        }
    }

    private void cr$ctxFillClipped(GuiGraphicsExtractor g, int x0, int y0, int x1, int y1, int color, int clipTop, int clipBot) {
    ...
    private void cr$renderConfirmModal(GuiGraphicsExtractor graphics, int screenW, int screenH, int mouseX, int mouseY) {
        ...
        graphics.centeredText(mc.font, title, cx + CONFIRM_W / 2, cy + CTX_PAD + 2, 0xFFFFFFFF);
        ...
        for (FormattedCharSequence line : lines) {
            graphics.centeredText(mc.font, line, cx + CONFIRM_W / 2, textY, 0xFFAAAAAA);
        ...
        graphics.centeredText(mc.font, yesLabel, yesX + btnW / 2, btnY + (CONFIRM_BTN_H - mc.font.lineHeight) / 2, 0xFFFFFFFF);
        graphics.centeredText(mc.font, noLabel, noX + btnW / 2, btnY + (CONFIRM_BTN_H - mc.font.lineHeight) / 2, 0xFFFFFFFF);
    }
