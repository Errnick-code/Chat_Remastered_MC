...
public final class DebugCommands {
            ...
            var clearcacheNode = literal("clearcache")
                    .executes(ctx -> {
                        ...
                        Thread thread = new Thread(() -> {
                            ...
                            mc.execute(() -> mc.gui.getChat().addClientSystemMessage(Component.literal(
    ...
    public static void debugTestConnection(Minecraft mc) {
        ...
        chat.addClientSystemMessage(Component.literal("§8[Chat Remastered] §7--- Connection test ---"));

        if (hasMod == null) {
            chat.addClientSystemMessage(Component.literal("§8[Chat Remastered] §cServer mod: §cnot detected (no handshake)"));
        } else {
            chat.addClientSystemMessage(Component.literal("§8[Chat Remastered] §aServer mod: §fv" + hasMod));
        }

        if (token.isEmpty()) {
            chat.addClientSystemMessage(Component.literal("§8[Chat Remastered] §cUpload token: §cnot received"));
        } else {
            chat.addClientSystemMessage(Component.literal("§8[Chat Remastered] §aUpload token: §freceived (" + token.length() + " chars)"));
        }

        chat.addClientSystemMessage(Component.literal("§8[Chat Remastered] §7TCP " + host + ":" + port + " — pinging..."));
        Thread thread = new Thread(() -> {
            ...
            mc.execute(() -> {
                if (ok) {
                    chat.addClientSystemMessage(Component.literal("§8[Chat Remastered] §aTCP: §aOK §7(" + ms + "ms)"));
                    chat.addClientSystemMessage(Component.literal("§8[Chat Remastered] §a✔ " + ChatRemasteredConfig.tr("chat-remastered.tcp_ok")));
                } else {
                    chat.addClientSystemMessage(Component.literal("§8[Chat Remastered] §cTCP: §ccannot connect to " + host + ":" + port));
                    chat.addClientSystemMessage(Component.literal("§8[Chat Remastered] §c✘ " + ChatRemasteredConfig.tr("chat-remastered.tcp_fail")));
                }
            });
        });
        thread.setDaemon(true);
