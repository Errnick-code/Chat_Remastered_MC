...
public final class NetworkPacketHandlers {
    ...
    private static void pingTcpServer(Minecraft mc) {
        ...
        mc.execute(() -> {
            if (ok) {
                mc.gui.getChat().addClientSystemMessage(Component.literal("§8[Chat Remastered] §a✔ " + ChatRemasteredConfig.tr(
                        "chat-remastered.connected", (Object) ("§7" + ChatRemasteredConfig.getResolution() + "§a"))));
            } else {
                mc.gui.getChat().addClientSystemMessage(Component.literal("§8[Chat Remastered] §c✘ " + ChatRemasteredConfig.tr(
    ...
    private static void registerImagePackets() {
        ...
        ClientPlayNetworking.registerGlobalReceiver(ImageErrorPacket.TYPE, (payload, context) ->
                context.client().execute(() -> {
                    ...
                    Minecraft.getInstance().gui.getChat().addClientSystemMessage(Component.literal("§8[Chat Remastered] " + msg));
                }));
    }
        ...
        ClientPlayNetworking.registerGlobalReceiver(PhotoDeniedPacket.TYPE, (payload, context) ->
                context.client().execute(() -> {
                    switch (payload.reason()) {
                        ...
                        default -> Minecraft.getInstance().gui.getChat().addClientSystemMessage(
