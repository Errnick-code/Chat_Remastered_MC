...
public final class ChatContextMenuRenderer {
    ...
    public static void render(GuiGraphicsExtractor graphics, Minecraft mc, int mouseX, int mouseY,
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (!icon.isEmpty()) {
                graphics.text(mc.font, icon, mx0 + PAD + 2, textY, textColor, false);
                graphics.text(mc.font, menuLabels[i], mx0 + PAD + 11, textY, textColor, false);
            } else {
                graphics.text(mc.font, menuLabels[i], mx0 + PAD + 2, textY, textColor, false);
            }
        }
    }

    public static void fillClipped(GuiGraphicsExtractor g, int x0, int y0, int x1, int y1, int color, int clipTop, int clipBot) {
