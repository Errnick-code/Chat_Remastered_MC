...
public final class EntityTagSuggestions {
    ...
    private static final String PREFIX_BLOCK = "<chat_remastered:block:";
    ...
    private static String[] blockIds() {
        java.util.List<String> ids = new ArrayList<>();
        for (net.minecraft.resources.Identifier id
    ...
    public static List<Suggestion> compute(String text, int cursor) {
        ...
        if (!head.startsWith(PREFIX_ROOT)) {
            if (PREFIX_ROOT.startsWith(head) && !head.isEmpty()) {
                addPrefixed(result, new String[] { PREFIX_ENTITY, PREFIX_PLAYER, PREFIX_ITEM, PREFIX_BLOCK }, head, "");
            }
            return result;
        }
        ...
        if (head.equals(PREFIX_ROOT)) {
            ...
            result.add(new Suggestion("block:", "block:"));
            return result;
        }
        ...
        if (head.startsWith(PREFIX_BLOCK)) {
            String tail = head.substring(PREFIX_BLOCK.length());
            ...
            if (!pathClosed) {
                ...
                if (!exactMatch) {
                    addPrefixed(result, blockIds, afterNamespace, "");
                    return result;
                }
