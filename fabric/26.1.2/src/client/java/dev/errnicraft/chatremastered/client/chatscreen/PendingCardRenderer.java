...
public final class PendingCardRenderer {
    ...
    public static int renderPendingCard(GuiGraphicsExtractor graphics, Minecraft mc, int mouseX, int mouseY,
    ...
    public static void renderPendingCardContent(GuiGraphicsExtractor graphics, Minecraft mc, int mouseX, int mouseY,
        ...
        if (!isLoaded || tex == null) {
            ...
            if (progress < 0f && dispH >= 6) {
                ...
                if (dispW >= 40 && barTop - previewTop >= mc.font.lineHeight + 2) {
                    ...
                    if (textY >= previewTop + 1)
                        graphics.text(mc.font, waitStr, textX, textY, tintAlpha(0xFFAAAAAA, a), false);
                }
                ...
                if (dispW >= 40 && barTop - previewTop >= mc.font.lineHeight + 2) {
                    ...
                    int textY = barTop - mc.font.lineHeight - 1;
                    if (textY >= previewTop + 1)
                        graphics.text(mc.font, pctStr, textX, textY, tintAlpha(0xFFFFFFFF, a), false);
                }
            }
            ...
            if (iconAreaH >= 12) {
                ...
                graphics.text(mc.font, icon, 0, 0, iconColor, false);
                graphics.pose().popMatrix();
            }
        } else {
            ...
            graphics.blit(RenderPipelines.GUI_TEXTURED, tex, 0, 0, 0f, 0f, texW, texH, texW, texH, tintAlpha(0xFFFFFFFF, a));
            graphics.pose().popMatrix();
        }
        ...
        graphics.centeredText(mc.font, "✕", closeX + closeSize / 2, closeY + (closeSize - mc.font.lineHeight) / 2, closeColor);
    }
    ...
    public static void renderRemovingCard(GuiGraphicsExtractor graphics, Minecraft mc, PendingCardAnimator.RemoveState r, int screenH) {
        if (r.shards != null) {
    ...
    public static void renderShatterCard(GuiGraphicsExtractor graphics, Minecraft mc, PendingCardAnimator.RemoveState r) {
    ...
    private static void renderShardTexture(GuiGraphicsExtractor graphics, PendingCardAnimator.RemoveState r, PendingCardAnimator.Shard s, int alphaByte) {
        int texW = r.texW;
        int texH = r.texH;
    ...
    private static void blitShardRun(GuiGraphicsExtractor graphics, PendingCardAnimator.RemoveState r, PendingCardAnimator.Shard s,
    ...
    public static void renderRemovingCardContent(GuiGraphicsExtractor graphics, Minecraft mc, PendingCardAnimator.RemoveState r,
        ...
        if (r.isLoaded && r.tex != null) {
            ...
            graphics.pose().translate(previewLeft + offsetX, previewTop + offsetY);
            graphics.pose().scale(s, s);
            graphics.blit(RenderPipelines.GUI_TEXTURED, r.tex, 0, 0, 0f, 0f, texW, texH, texW, texH, tintAlpha(0xFFFFFFFF, alphaByte));
            graphics.pose().popMatrix();
        } else {
