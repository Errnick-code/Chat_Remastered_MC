...
public final class ChatLineLocator {
    ...
    public static void drawLineHighlight(GuiGraphics graphics, Minecraft mc, int lineIdx) {
        ChatComponent chat = mc.gui.getChat();
        ChatComponentAccessor acc = (ChatComponentAccessor) chat;
        List<GuiMessage.Line> trimmed = acc.getTrimmedMessages();
        int scrollPos = acc.getChatScrollbarPos();
    ...
    public static void renderHighlight(GuiGraphics graphics, Minecraft mc, HighlightState highlight, int highlightDurationMs) {
    ...
    public static int updateCursorAndHover(GuiGraphics graphics, Minecraft mc, ChatScreen self, int mouseX, int mouseY,
        ...
        for (ChatRemasteredStore.ImageMessage msg : ChatRemasteredStore.getMessageList()) {
            ...
            for (var entry : msg.getRowCardBounds().entrySet()) {
                ...
                if (mouseX >= b[0] && mouseX < b[2] && mouseY >= b[1] && mouseY < b[3]) {
                    ...
                    if (!isDeleted && !isError)
                        GLFW.glfwSetCursor(mc.getWindow().getWindow(), GLFW.glfwCreateStandardCursor(GLFW.GLFW_POINTING_HAND_CURSOR));
                    else
