                ...
                .thenCompose(response -> {
                    ...
                    return CLIENT.sendAsync(texturesRequest, HttpResponse.BodyHandlers.ofString())
                            .thenCompose(texResp -> {
                                ...
                                PropertyMap propertyMap = new PropertyMap(propsBuilder.build());
                                chatDebug("skin resolved OK for '" + playerName + "', properties=" + propertyMap.size());
                                return CompletableFuture.completedFuture(new GameProfile(uuid, playerName, propertyMap));
                            });
                })
                .exceptionally(ex -> {
    ...
    private static CompletableFuture<GameProfile> fetchViaPlayerDb(String playerName) {
        ...
        return CLIENT.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                .thenApply(response -> {
                    ...
                    PropertyMap propertyMap = new PropertyMap(propsBuilder.build());
                    return new GameProfile(uuid, playerName, propertyMap);
                })
                .exceptionally(ex -> {
